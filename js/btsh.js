...
function btsh(baseurl, tournament_key) {
	...
	function send_tshirt_sizes(s, match_id, players, cb) {
		...
		var raw_id = match_id.substring('bts_'.length);
		var url = baseurl + 'h/' + encodeURIComponent(tournament_key) + '/m/' + encodeURIComponent(raw_id) + '/tshirt-sizes';
		_request_json(s, 'btsh.tshirt', {
			method: 'POST',
			url: url,
	...
	function reload_match_information() {
		if (state.ui.resultmode_visible) {
			// Keep court_id as-is (set from URL hash)
		} else if (state.ui.displaymode_visible) {
